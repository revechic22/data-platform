...
import pdfplumber
import fitz  # 추가: 이미지 추출용 (PyMuPDF)
from PIL import Image  # 추가: 이미지 핸들링
import re
import json
import io
import pandas as pd
...
def init_session_state():
    if "current_step" not in st.session_state:
        st.session_state.update({
            ...
            "section_blocks": {
                "1.1": [
                    {"type": "text", "label": "1) 사업의 특성", "content": ""},
                    {"type": "table", "label": "2) 내용 및 범위", "content": pd.DataFrame([["위치", ""], ["과업기간", ""]], columns=["구분", "내용"])},
                    {"type": "image", "label": "위치도/전경", "content": None}
                ],
                "1.2": [{"type": "text", "label": "1) 지역 현황", "content": ""}],
                "1.3": [{"type": "text", "label": "1) 관련 계획 검토", "content": ""}],
                "2.1": [{"type": "text", "label": "1) 기술인력 투입계획", "content": ""}],
                "2.2": [{"type": "text", "label": "1) 업무 수행 조직", "content": ""}],
                "2.3": [{"type": "text", "label": "1) 업무 분담 체계", "content": ""}],
                "2.4": [{"type": "text", "label": "1) 투입 일정 계획", "content": ""}]
            }
        })

def analyze_smart_consistency(user_text, task_info):
    """
    고정된 if-else가 아닌, 과업지시서 내용 기반 유연한 대조
    """
    scopes = task_info.get("과업범위", [])
    matched = []
    missing = []
    
    # 과업지시서 범위에 있는 단어가 본문에 있는지 체크
    for scope in scopes:
        # '제1편 상수도정비' -> '상수도' 추출
        core_word = re.sub(r'제\d+편\s*', '', scope).strip()[:3] 
        if core_word in user_text:
            matched.append(core_word)
        else:
            missing.append(core_word)
            
    # 용어 준수 체크 (사업책임기술인)
    wrong_term = "사업책임기술자"
    term_violation = user_text.count(wrong_term)
    
    return {"matched": matched, "missing": missing, "term_violation": term_violation}
...
def process_pdfs(task_file, qbs_files):
    """과업지시서 파싱 및 참고 QBS에서 이미지 추출"""
    # 1. 과업지시서 파싱 (기본 정보 및 과업범위 추출)
    if task_file:
        # 기존에 작성하신 extract_text_from_pdf를 호출합니다.
        text = extract_text_from_pdf(task_file)
        # 이 부분은 기존에 사용하시던 파싱 함수(예: parse_task_pdf)가 있다면 연결하세요.
        # 없더라도 task_info 구조를 유지하기 위해 아래와 같이 설정합니다.
        if "task_info" not in st.session_state:
            st.session_state.task_info = {"과업범위": []}
        
        # 텍스트에서 '제N편'으로 시작하는 목차를 찾아 과업범위로 인식 (정합성 체크용)
        scopes = re.findall(r'제\d+편\s+[^\n]+', text)
        st.session_state.task_info["과업범위"] = scopes

    # 2. 참고 QBS 이미지 추출 (PyMuPDF 활용)
    all_imgs = []
    for qf in qbs_files:
        # 파일 포인터를 처음으로 되돌립니다.
        qf.seek(0)
        doc = fitz.open(stream=qf.read(), filetype="pdf")
        for i, page in enumerate(doc):
            # 페이지 내 모든 이미지 탐색
            for img_index, img in enumerate(page.get_images()):
                xref = img[0]
                pix = fitz.Pixmap(doc, xref)
                # RGBA를 RGB로 변환 (투명도 처리)
                ...
                all_imgs.append({
                    ...
                    "page": i+1, 
                    "img": img_data,
                    "id": f"{qf.name}_{i}_{img_index}"
                })
        doc.close()
    st.session_state.extracted_images = all_imgs
...
def parse_task_pdf(text):
    """추출된 텍스트에서 사업명, 위치, 기간, 범위를 정규식으로 추출"""
    info = {
        "사업명": "미확인",
        "위치": "미확인",
        "과업기간": "미확인",
    ...
    # 1. 사업명 추출 (보통 첫 줄이나 '용역명' 뒤에 옴)
    name_match = re.search(r'(?:사업명|용역명)[:\s]+([^\n]+)', text)
    if name_match:
        info["사업명"] = name_match.group(1).strip()
        
    # 2. 위치 추출
    loc_match = re.search(r'(?:위치|지점)[:\s]+([^\n]+)', text)
    if loc_match:
        info["위치"] = loc_match.group(1).strip()
        
    # 3. 과업기간 추출
    period_match = re.search(r'(?:과업기간|용역기간)[:\s]+([^\n]+)', text)
    if period_match:
        info["과업기간"] = period_match.group(1).strip()

    # 4. 과업범위 (제N편 또는 핵심 키워드 추출)
    # 승민 님이 걱정하신 '상수/하수' 구분을 위해 목차 형태를 수집합니다.
    scopes = re.findall(r'제\d+편\s+([^\n]+)', text)
    if not scopes:
        # 목차 형식이 없을 경우 주요 키워드라도 수집
        keywords = ["상수도", "하수도", "처리장", "관로", "펌프장", "침수"]
        scopes = [k for k in keywords if k in text]
        
    info["과업범위"] = list(set(scopes)) # 중복 제거
...
def render_editor(section_id):
    """
    승민 님을 위한 2단 구성 에디터:
    [왼쪽] 과업범위 대조 + 이미지 갤러리 / [오른쪽] 실제 QBS 작성 및 테이블 편집
    """
    st.header(f"📍 {section_id} 섹션 설계 및 정합성 검토")
    ...
    # --- [왼쪽: 검토 및 참고 영역] ---
    with col_ref:
        st.subheader("🔍 실시간 과업 정합성")
        
        # 현재 작성 중인 모든 텍스트를 합쳐서 분석 엔진에 전달
        current_text = ""
        for block in st.session_state.section_blocks.get(section_id, []):
            if block['type'] == 'text':
                current_text += str(block['content'])
        
        # 아까 만든 유연한 정합성 엔진 호출
        analysis = analyze_smart_consistency(current_text, st.session_state.task_info)
        
        # 1. 과업 범위 대조 결과 (상수/하수/관로 등)
        if analysis['matched']:
            st.success(f"✅ 과업지시서 범위 반영 확인: {', '.join(analysis['matched'])}")
        
        if analysis['missing']:
            st.info(f"💡 미반영 과업 범위(참고): {', '.join(analysis['missing'])}")
            st.caption("※ 과업 범위에 관로가 포함되어 있다면 관로 내용을 작성해도 안전합니다.")

        # 2. 필수 용어 체크 (사업책임기술인)
        if analysis['term_violation'] > 0:
            st.error(f"⚠️ 용어 수정 필요: '사업책임기술자' → '사업책임기술인' ({analysis['term_violation']}건)")

        # 3. 참고 이미지 갤러리 (PDF에서 뽑아온 그림들)
        st.divider()
        st.subheader("🖼️ 참고 QBS 이미지")
        if st.session_state.extracted_images:
            for i, img_obj in enumerate(st.session_state.extracted_images[:5]): # 상위 5개 우선 노출
                with st.expander(f"이미지: {img_obj['name']} (p.{img_obj['page']})"):
                    st.image(img_obj['img'], use_column_width=True)
                    if st.button(f"이 슬롯에 복사", key=f"copy_img_{i}"):
                        # 이미지 섹션(보통 3번째 블록)에 이미지 삽입
                        st.session_state.section_blocks[section_id][2]['content'] = img_obj['img']
                        st.rerun()
        else:
            st.write("추출된 참고 이미지가 없습니다.")

    # --- [오른쪽: 실제 작성 영역] ---
    with col_edit:
        st.subheader("✍️ QBS 상세 작성")
        if section_id in st.session_state.section_blocks:
            for idx, block in enumerate(st.session_state.section_blocks[section_id]):
                ...
                if block['type'] == 'text':
                    block['content'] = st.text_area(
                        "내용 입력", 
                        value=block['content'], 
                        key=f"input_{section_id}_{idx}", 
                        height=250,
                        label_visibility="collapsed"
                    )
                elif block['type'] == 'table':
                    # 표 형태의 데이터 편집 (위치, 기간 등)
                    block['content'] = st.data_editor(
                        block['content'], 
                        key=f"table_{section_id}_{idx}", 
                        use_container_width=True
                    )
                elif block['type'] == 'image' and block['content']:
                    st.image(block['content'], caption="선택된 이미지", use_column_width=True)
                    if st.button("이미지 삭제", key=f"del_{section_id}_{idx}"):
                        block['content'] = None
                        st.rerun()
